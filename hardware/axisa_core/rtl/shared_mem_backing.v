...
// endianness convention). Ported byte-for-byte from
// rv32i_core/rtl/data_mem.v to back noc_mem_adapter.v (also ported
// unmodified - see cpu_core.v's bus-master-port comment) - renamed to
// shared_mem_backing to avoid colliding with THIS project's own
// data_mem.v (word-addressed, a completely different module with an
// unrelated port shape, used for AxISA's private per-core memory).
// AxISA's cpu_core.v always drives mem_size=2'b10 (word) for every
// shared-region access (v0.1 has no partial-word LOAD/STORE at all),
// so byte_read/half_read below are dead code from AxISA's own core,
// still present only because this module is a verbatim, unmodified
// copy - kept that way deliberately, not trimmed, so there is exactly
// one place to compare against if rv32i_core's own copy ever changes.
// Combinational read (same reasoning as instr_mem.v - single-cycle
// datapath, no separate memory stage); writes land on the clock edge
// with byte-lane masking so a narrow store never touches neighboring
// bytes.
`timescale 1ns/1ps

module shared_mem_backing #(
