...
// hardware/rv32i_core/rtl/fp_addsub.v (that project's real, working
// RV32F FPU) - used here by cpu_core.v's QHAD amplitude-mixing
// sequencer, AxISA's own arithmetic (alu.v) has no multiply/float of
// any kind. Scope limitation, deliberate, inherited unchanged: every
// input is assumed to be either a normal finite number (biased
// exponent in [1,254]) or exact zero. NaN, infinity, and subnormal
// inputs are out of scope and may produce numerically wrong (but not
// X-propagating or hanging) results - see fp_mul.v's own comment for
// the same note.
`timescale 1ns/1ps
