...
echo.
echo ===== Mini-SoC: AxISA's FIRST multi-core mesh (2x2, 3 cores + 1 memory) =====
...
if %errorlevel% neq 0 goto :error
echo c0 (consumer, busy-waits) reads c1's (producer) payload through the arbitrated NoC - only correct (127) if it genuinely observed the other core's write; c2 runs independently (test1.hex) alongside
...
echo   OK (c0=127, c1=77, c2=200, cross-core communication verified)
