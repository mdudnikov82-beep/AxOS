...
module tb_forward_unit;
    ...
    initial begin
        ...
        id_ex_rs1 = 5'd3; ex_mem_rd = 5'd3; mem_wb_rd = 5'd3; id_ex_rs2 = 5'd0;
        #1;
        ...
        // x0 must never be forwarded even if addresses "match" on 0.
        id_ex_rs1 = 5'd0; ex_mem_rd = 5'd0; ex_mem_reg_write = 1;
        #1;
        check(forward_a, 2'b00, "x0 never forwarded");

        if (errors == 0) $display("ALL FORWARD_UNIT TESTS PASSED");
        else $display("%0d FORWARD_UNIT TESTS FAILED", errors);
        $finish;
    end
endmodule
