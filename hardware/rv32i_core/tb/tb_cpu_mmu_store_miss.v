// Regression test for the store/TLB-miss physical-page-0 corruption bug
// (see sw/asm_mmu_store_miss_test.py and cpu_core.v's dmem_write comment
// for the full story) - found live: !mmu_walk_active alone doesn't cover
// the S_IDLE miss-detection cycle or the S_FILL cycle, so a first-touch
// store used to also write its data to physical page 0 (at the faulting
// vaddr's own page offset) in addition to landing correctly at the real
// translated address. Fixed by also gating dmem_write on !mmu_stall.
`timescale 1ns/1ps
