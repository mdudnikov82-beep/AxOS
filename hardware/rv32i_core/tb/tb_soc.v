// SoC-level testbench: one shared clk/reset drives all 71 cores in
// soc_top at once (the 4D 2x3x6x2/72-node mesh - 36 P-cores +
// 35 E-cores + 1 memory node; just more core instantiations, soc_top.v
// itself is a 4-dimensional mesh, but this testbench only ever sees the
...
// here - that cross-core-communication proof, including the mandatory
// real W-axis hop, lives in tb_shared_soc.v instead). Waits for
...
module tb_soc;
    ...
    initial begin
        ...
        if (!all_halted) begin
            ...
            if (!any_fail) $display("PASS: all 71 cores matched their expected values");
        end
