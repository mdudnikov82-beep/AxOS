// Heterogeneous SoC: 3 P-cores (cpu_core_pipelined, the 5-stage
// pipeline - performance) + 3 E-cores (cpu_core, the single-cycle
// design - efficiency), each with its own private instruction/data
// memory PLUS a shared memory region all six can reach through the
// N-way round-robin shared_bus - scaled up from the earlier 2P+2E SoC
// purely by instantiating two more cores and widening the flattened bus
// vectors; shared_bus.v itself needed NO changes (already genuinely
// N-way, not hard-coded to any particular count). Core index mapping
// onto the bus: 0=p0, 1=p1, 2=p2, 3=e0, 4=e1, 5=e2 (P-cores first, then
// E-cores) - an arbitrary but fixed, documented convention.
`timescale 1ns/1ps
...
module soc_top #(
    ...
    localparam NUM_CORES = 6;

    wire        p0_bus_req, p1_bus_req, p2_bus_req, e0_bus_req, e1_bus_req, e2_bus_req;
    wire [31:0] p0_bus_addr, p1_bus_addr, p2_bus_addr, e0_bus_addr, e1_bus_addr, e2_bus_addr;
    wire [31:0] p0_bus_write_data, p1_bus_write_data, p2_bus_write_data, e0_bus_write_data, e1_bus_write_data, e2_bus_write_data;
    wire        p0_bus_mem_write, p1_bus_mem_write, p2_bus_mem_write, e0_bus_mem_write, e1_bus_mem_write, e2_bus_mem_write;
    wire [1:0]  p0_bus_mem_size, p1_bus_mem_size, p2_bus_mem_size, e0_bus_mem_size, e1_bus_mem_size, e2_bus_mem_size;
    wire        p0_bus_mem_unsigned, p1_bus_mem_unsigned, p2_bus_mem_unsigned, e0_bus_mem_unsigned, e1_bus_mem_unsigned, e2_bus_mem_unsigned;
    wire        p0_bus_grant, p1_bus_grant, p2_bus_grant, e0_bus_grant, e1_bus_grant, e2_bus_grant;
    ...
    // 0=p0,1=p1,2=p2,3=e0,4=e1,5=e2.
    shared_bus #(.NUM_CORES(NUM_CORES), .MEM_BYTES(SHARED_MEM_BYTES)) sbus (
        .clk(clk), .reset(reset),
        .req({e2_bus_req, e1_bus_req, e0_bus_req, p2_bus_req, p1_bus_req, p0_bus_req}),
        .addr_flat({e2_bus_addr, e1_bus_addr, e0_bus_addr, p2_bus_addr, p1_bus_addr, p0_bus_addr}),
        .write_data_flat({e2_bus_write_data, e1_bus_write_data, e0_bus_write_data, p2_bus_write_data, p1_bus_write_data, p0_bus_write_data}),
        .mem_write({e2_bus_mem_write, e1_bus_mem_write, e0_bus_mem_write, p2_bus_mem_write, p1_bus_mem_write, p0_bus_mem_write}),
        .mem_size_flat({e2_bus_mem_size, e1_bus_mem_size, e0_bus_mem_size, p2_bus_mem_size, p1_bus_mem_size, p0_bus_mem_size}),
        .mem_unsigned({e2_bus_mem_unsigned, e1_bus_mem_unsigned, e0_bus_mem_unsigned, p2_bus_mem_unsigned, p1_bus_mem_unsigned, p0_bus_mem_unsigned}),
        .grant({e2_bus_grant, e1_bus_grant, e0_bus_grant, p2_bus_grant, p1_bus_grant, p0_bus_grant}),
        .read_data(shared_read_data)
    );

    assign all_halted = p0_halted && p1_halted && p2_halted && e0_halted && e1_halted && e2_halted;
endmodule
