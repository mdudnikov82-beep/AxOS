...
## AI-демо: нейросеть учит XOR (`ai.bin`, `src/user/ai.c`)
...
XOR. XOR не линейно разделим — однослойный перцептрон в принципе не может
его выучить (классический контрпример Минского/Паперта 1969 года), отсюда
и скрытый слой.
...
- **Сделано:**
  ...
  - ✅ AI-демо (`ai.bin`, `src/user/ai.c`): крошечный MLP (2-4-1) учится
    предсказывать XOR обратным распространением ошибки, целиком на
    fixed-point Q16.16 (без FPU) — см. "AI-демо" выше.
