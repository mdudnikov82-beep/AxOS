...
`src/fs/fat12.c`/`fat12.h` — драйвер FAT12-раздела, который загрузчик кладёт
в RAM по адресу `0x20000` (64 КБ, 128 секторов, 125 кластеров данных по
512 Б). Сам раздел собирается из файлов каталога `fs/` скриптом
...
и `set_fat_entry` в `tools/make_fat12.py`. Изменения живут только в RAM
текущей сессии (как и весь RAM-диск).
...
символы — независимо от обычного ввода shell.
...
| `write <file> <text>` | создать/перезаписать файл на FAT12 RAM-диске         |
...
- ✅ FAT12 RAM-диск (чтение и запись) + команды `ls`/`cat`/`write`.
