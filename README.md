...
сохраняет веса (вместе с обученным паттерном) на FAT12 в `AI.WTS`
(`save_weights()`/`load_weights()`) — следующий запуск `ai` с тем же
...
прогресс-бара (баннер показывает `Loaded:` вместо `Training:`). Запись
требует разблокированного диска (команда `unlock` в AxSH — FAT12
заблокирован на запись по умолчанию, `fat12_locked=1`, см. "Файловая
система" выше); если диск заблокирован, `ai` просто предупреждает и
работает как раньше — чтение `AI.WTS` не требует `unlock`. Несошедшуюся
модель (`all_ok=0`) не сохраняет.
