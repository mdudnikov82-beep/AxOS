...
API: `fat12_list()` (команда `ls`), `fat12_cat(filename)` (команда `cat`) и
`fat12_load(filename, buffer, max_size)` (команда `run`, см. ниже).

### Запуск пользовательских программ (run)

Команда `run <file>` грузит файл с FAT12-диска по фиксированному адресу
`0x100000` (1 МБ) — `fat12_load()` — и запускает его как ring3-задачу
`task_create_user(filename, (void(*)(void))0x100000)`. Область `0x100000`
внутри identity-mapped 4 МБ уже `PRESENT|USER|RW` (см. `paging.c`), поэтому
никаких изменений в paging для этого не нужно.
...
`user.ld` — линковка на `0x100000`), собираются и стрипуются в
`build/hello.bin`, копируются в `fs/HELLO.BIN` и попадают в FAT12-образ
вместе с остальными файлами `fs/`. Это не настоящий ELF-парсер (сегменты
ELF не разбираются), а прагматичный аналог: тот же `objcopy -O binary`,
что и для ядра, но на отдельный фиксированный адрес.

Ограничение: один фиксированный слот `0x100000` (до `USER_PROGRAM_MAX_SIZE`
= 32 КБ) — повторный `run` во время работы предыдущей программы перезапишет
её код. Для учебной ОС без отдельных адресных пространств на задачу это
ожидаемо.
...
| `0x100000`         | ≤ 32 КБ    | Загрузка пользовательских программ (команда `run`, `fat12_load`) |
...
- ⏳ Запись в FAT12, больше syscall'ов для ring3 — см. Roadmap.
...
- **Сделано:**
  ...
  - ✅ загрузчик пользовательских программ из FAT12 (команда `run`,
    плоские бинарники на фиксированный адрес `0x100000`, см. `src/user/`).
...
- **Среднесрочно:**
  ...
  - несколько слотов/областей памяти для одновременно запущенных
    пользовательских программ (сейчас один фиксированный слот `0x100000`).
