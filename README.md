...
**Ограничение**: `page_fault_handler_main` (`paging.c`) при любом page fault
останавливает ВСЮ систему (`halt`), а не только задачу, его вызвавшую —
поэтому изолированная задача, вышедшая за пределы своего окна, "падает" вместе
со всей ОС. Корректное завершение отдельной задачи при ошибке — отдельная
будущая задача (требует механизма завершения/жизненного цикла задач).
...
- **Долгосрочно:**
  ...
  - корректное завершение задач (task exit) при page fault или нормальном
    выходе — сейчас любой page fault останавливает всю систему
    (`page_fault_handler_main`), в том числе у изолированных задач.
