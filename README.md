...
(`kernel.bin` грузится с `0x1000`), а GDT загрузочного сектора живёт по
`0x7c00`-`0x7e00` (см. предупреждение в `kernel_entry.asm`); 32 КБ
статического буфера в `.bss` утащили `__bss_end` далеко за `0x7c00`, и
`rep stosb`, обнуляющий `.bss` при старте, стирал GDT — `#GP` → `#DF` →
Triple fault прямо во время загрузки. Тот же класс бага, что когда-то уже
ловили на самом `.bss` ядра. При добавлении новых крупных статических
буферов в `kernel.c` — фиксированный физический адрес в свободной зоне
(как `PAGE_DIRECTORY`/`PAGE_TABLE`/`PD_POOL_BASE`/`PT_POOL_BASE`), не
`.bss`.
...
![ai 0110: прогресс-бар обучения и таблица предсказаний для XOR](docs/screenshots/ai-xor-training.png)

*`ai` без аргументов (интерактивный ввод, Enter = XOR по умолчанию):
прогресс-бар обучения (красный → жёлтый → зелёный по мере убывания
ошибки) и финальная таблица предсказаний.*

![ai 0001: AND обучается быстрее XOR, т.к. линейно разделима](docs/screenshots/ai-argv-and.png)

*`ai 0001` (AND через аргумент командной строки) — сходится заметно
быстрее XOR, т.к. AND линейно разделима и не нуждается в скрытом слое
теоретически (но архитектура одна для всех 16 функций).*

![ai 0011: "A" (выход не зависит от B) сходится почти мгновенно](docs/screenshots/ai-argv-0011.png)

*`ai 0011` (функция "A", выход = первый вход, второй вообще не влияет)
— тривиально линейно разделима, ошибка уже зелёная к 500-й эпохе.*

`ai` (запускается прямо из AxSH или `run AI.BIN` из kernel-shell) — живое
обучение крошечного многослойного перцептрона (2 входа → 4 скрытых
нейрона → 1 выход, обучение обратным распространением ошибки) предсказывать
любую из 16 булевых функций двух аргументов. Таблица истинности (4 бита —
выходы для входов 00,01,10,11) задаётся либо аргументом командной строки
(`ai 0110` — XOR, `ai 0001` — AND, `ai 0111` — OR, и т.д.; невалидный
аргумент — 4 символа `0`/`1` обязательны — печатает предупреждение и
откатывается на XOR), либо интерактивно: без аргументов `ai` запрашивает
4-битную строку через `ax_readline` (Enter без ввода — тоже XOR). Перед
обучением печатает распознанное имя функции (AND/OR/XOR/NAND/NOR/XNOR/...,
таблица `KNOWN_FNS` покрывает все 16 — lookup по 4 битам всегда находит
совпадение). XOR/XNOR — самые сложные из 16: не линейно разделимы —
однослойный перцептрон в принципе не может их выучить (классический
контрпример Минского/Паперта 1969 года), отсюда и скрытый слой, которого
достаточно для всех 16 функций.
...
свои случайные веса. С 2 скрытыми нейронами обучение иногда застревает в
локальном минимуме (проверено на 30 случайных сидах в Python-прототипе:
25/30 сходится) — 4 нейрона дают избыточность, сходится 200/200 на
проверке.
...
а реальные линии. Каждые 500 эпох (5000 всего) печатается живой
...
рамкой (те же box-drawing символы, `PRED_TABLE_WIDTHS`) с колонками
A/B/predicted/target/status, `OK`/`FAIL` подсвечены зелёным/красным;
выравнивание ячеек — через свой `print_padded()` (ширина берётся из
переменной, а не из литерала формата, как требует `%-Ns` у `ax_printf`).
Цвет в ячейке `status` не портит выравнивание: ANSI-escape'ы печатаются
отдельными вызовами вокруг `print_padded()`, а не внутри её аргумента,
так что в видимую ширину поля не попадают.
...
- **Сделано:**
  ...
  - ✅ AI-демо (`ai.bin`, `src/user/ai.c`): крошечный MLP (2-4-1) учится
    предсказывать любую из 16 булевых функций двух аргументов (таблица
    истинности — аргументом командной строки или интерактивно) обратным
    распространением ошибки, целиком на fixed-point Q16.16 (без FPU); плюс
    отдельный режим `ai ask` — Q&A по жёстко заданной базе про сам AxOS
    (поиск подстроки, без ML) — см. "AI-демо" выше.
