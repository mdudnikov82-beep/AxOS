...
// Портирован с RISC-V стороны (src/user/rv64/dhcp.h) - тот же протокол,
// та же логика.
//
...
static void dhcp_send(unsigned char type, unsigned int xid, const unsigned char my_mac[6],
    ...
    ax_net_send(dhcp_tx, 14 + ip_total);
}

// Разбирает опции DHCP-сообщения (dhcp - указатель на его начало, n - его
// полную длину в байтах, включая BOOTP-заголовок). Проверяет xid и magic
...
// Поллит ax_net_recv() до timeout_ms, ища DHCP-ответ типа want_type с
// нашим xid, адресованный DHCP_CLIENT_PORT. При успехе кладёт
// yiaddr/server_id (любой указатель можно передать NULL) и возвращает 1.
static int dhcp_wait_for(unsigned char want_type, unsigned int xid, unsigned int timeout_ms,
    ...
    while (waited < timeout_ms) {
        unsigned int n = ax_net_recv(dhcp_rx, sizeof(dhcp_rx));
