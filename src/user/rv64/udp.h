...
// ip_checksum()/MY_IP/arp_resolve(), сам ICMP тут не нужен, но выносить его
// в третий header ради этого не стоит). Как arp.h/icmp.h - header-only,
// static-состояние, своя копия на программу.
...
// тегов поколения (malloc.h) - переиспользовано вместо изобретения
...
static unsigned int net_rand32(void) {
    if (__net_rand_state == 0) {
        __net_rand_state = (unsigned long long)gettime() ^ 0x9E3779B97F4A7C15ULL;
...
// 0 означает "не считалась", это не битый пакет, а легальный режим (экономит
// вторую контрольную сумму, которая ещё и по псевдозаголовку с IP-адресами).
static int udp_send(unsigned int dst_ip, unsigned short dst_port, unsigned short src_port,
    ...
    return (net_send(udp_tx, 14 + ip_total) == 0) ? 0 : -1;
}

// Неблокирующий приём одной UDP-датаграммы (poll - как net_recv()). Если
// что-то пришло и это UDP, копирует payload в buf (обрезая по max_len),
// заполняет src_ip/src_port/dst_port (любой из указателей можно передать
// NULL) и возвращает длину скопированного payload. 0 - ничего/не UDP.
static unsigned int udp_recv(unsigned int *src_ip_out, unsigned short *src_port_out,
    ...
    unsigned int n = net_recv(udp_rx, sizeof(udp_rx));
    ...
    // РЕАЛЬНЫЙ БАГ (найден при аудите, не жил здесь случайно - tcp.h/
    // icmp.h уже делают эту же проверку, и комментарий у tcp_parse_segment
    // прямо утверждает, что udp.h/icmp.h "тоже не доверяют длине из
    // пакета сверх реально принятого" - тут это было НЕПРАВДОЙ). ihl
    // может быть больше 20 (IP-опции) - без этой проверки udp[4]/udp[5]
    // читались бы за пределами того, что n подтверждает, если пакет
    // короче, чем заявляет свой же IHL - т.е. как минимум "чужие" байты
    // от предыдущего кадра в udp_rx трактуются как новый UDP-заголовок,
    // а с достаточно большим max_len у будущего вызывающего - выход за
    // пределы самого массива udp_rx[600].
