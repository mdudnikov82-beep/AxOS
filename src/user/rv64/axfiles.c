...
int main(void) {
    ...
    window_t win;
    /* min_w=460 not 300 - files_layout()'s SIZE column sits at a fixed
     * offset from the window's right edge; a full 8.3 filename (up to
     * "XXXXXXXX.XXX", 12 chars) needs ~192px, and win.w-140 is the real
     * space available before SIZE - found live via screendump: at 300
     * the two columns visibly overlapped/merged on longer filenames. */
    window_init(&win, 100, 90, 600, 430, 460, 200, gfx_rgb(80, 200, 120), gfx_rgb(10, 25, 15),
               "AxFiles");
    ...
    for (;;) {
        ...
        if (mouse_state(&mx, &my, &buttons, &focused, &wheel)) {
            ...
            if (!dragging && !resizing && left && !prev_left && focused && window_hit_close(&win, mx, my)) {
                 ...
                 * below - the resize grip's corner geometrically overlaps
                 * the last file row's click region. */
                resizing = 1;
