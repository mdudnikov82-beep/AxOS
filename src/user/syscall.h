...
// в обход syscall_dispatch (нужен прямой доступ к живому регистровому
// кадру вызывающего, не просто ESI-аргумент - см. sys_fork_impl,
// kernel.c). БЕЗ arg-структуры: ESI не используется, результат приходит
// напрямую в EAX - 0 в потомке, pid потомка (>0) в родителе, -1 при
// ошибке (нет свободных слотов, или вызов не из изолированной задачи).
#define SYS_FORK 0x2A
