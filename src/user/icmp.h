...
// ни других типов ICMP. Портирован с RISC-V стороны (src/user/rv64/
// icmp.h) - тот же протокол, та же логика, только net_mac/net_send/
// net_recv/sleep_ms заменены на ax_net_mac/ax_net_send/ax_net_recv/
// ax_sleep_ms, а gettime() (10МГц CLINT) - на ax_get_ticks() (100Гц PIT).
...
// возвращает 1. Учитывает IHL заголовка IP и обрезает payload по
// вместимости icmp_tx и по факту реально принятых байт.
static int icmp_maybe_respond(const unsigned char *rx, unsigned int n) {
    ...
    ax_net_send(icmp_tx, 14 + reply_ip_total);
    return 1;
}

// Собирает и шлёт один ICMP echo request на dst_ip (резолвя MAC через
// arp_resolve_next_hop), ждёт ответ до timeout_ms (поллинг ax_net_recv()
// каждые 10мс). При успехе возвращает 1 и кладёт round-trip время (мс,
// округлённое до 10мс - разрешение 100Гц PIT-таймера) в *rtt_ms_out.
static int icmp_ping(unsigned int dst_ip, unsigned short seq,
    ...
    ax_net_send(icmp_tx, 14 + ip_total);

    unsigned int waited = 0;
    while (waited < timeout_ms) {
        unsigned int n = ax_net_recv(icmp_rx, sizeof(icmp_rx));
        if (n >= 14 + 20 + 8) {
            ...
            if (icmp_rx[12] == 0x08 && icmp_rx[13] == 0x00 &&   // ethertype IPv4
                ...
                if (rid == ICMP_ID && rseq == seq) {
                    if (rtt_ms_out) *rtt_ms_out = (unsigned long)(ax_get_ticks() - t0) * 10UL;
                    return 1;
                }
            }
