...
//
// KASLR-lite: база кучи больше не константа. Полноценный KASLR (случайный
// адрес ЗАГРУЗКИ самого ядра) тут недостижим без position-independent
...
// переделка. Зато конкретную структуру, которую типичный heap-баг
// (overflow/UAF) реально бьёт, можно сдвигать без переноса самого ядра:
// g_kheap_base выбирается случайно при каждой загрузке в init_paging()
// (см. paging.c) - PD[3..33] (floor был 2, сдвинут на 3 когда FAT12
// выросла до 2МБ и заняла PD[2] - см. FAT12_BASE в fat12.c), т.е. любое
// 2МБ-выровненное место в этом диапазоне, 31 вариант. Эксплойт,
// захардкоженный под 0x400000 (старое фиксированное значение),
// промахивается мимо кучи на следующей загрузке.
...
// НЕ пересекаются с диапазоном кандидатов кучи [PD[3], PD[34)), даже с
