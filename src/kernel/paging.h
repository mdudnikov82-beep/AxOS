...
// каждой загрузке в init_paging() (см. paging.c) - PD[3..47] (диапазон
// расширен с [3,34) - см. подробный разбор у kheap_pick_pd_index() в
// paging.c за тем, ПОЧЕМУ дальше расширить нельзя без развязки VA/PA
// (пробовали - нашли реальный double fault, откатили). Куча остаётся
...
// см. её комментарий у g_kheap_base; кандидаты [PD[48], PD[64)) намеренно
// НЕ пересекаются с диапазоном кандидатов кучи [PD[3], PD[48)), даже с
