...
//  1) выбирает случайный delta (кратный 4096, от 0 до свободного места)
...
//
// Энтропия: ~(max_segment_end_offset - code_size - 20КБ на кучу) / 4096
// позиций - НЕ 16, несмотря на что можно подумать по формуле выше:
// гранулярность обязана быть страничной (4096), а не побайтовой/16-байтной
// - delta прибавляется и к границе W^X (wx_data_off), а PAE-страницы не
// умеют иметь разные права доступа внутри одной физической страницы,
// так что граница код/данные обязана остаться выровненной на страницу
// после сдвига. Настоящая оценка при аудите KASLR: для hello (27KB) при
// 63KB окне минус 20KB на кучу - это ~3-4 позиции, ~2 бита, НЕ 11 (это
// расхождение нашли и задокументировали позже - код никогда не менялся,
// была неверна только эта оценка). Увеличить энтропию без нарушения
// W^X можно только увеличив само окно (USER_ARGS_OFFSET) - не сделано.
