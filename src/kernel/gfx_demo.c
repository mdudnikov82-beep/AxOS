// =================================================================
//  Графическая демка AxOS - mode 13h (320x200, 256 цветов)
// =================================================================
//
...
// масштабируем их под 320x200 при отрисовке курсора.
...
void gfx_main() {
    ...
    fill_screen(0x01); // синий фон (стандартная палитра VGA в mode 13h)
    ...
    while (1) {
        ...
        if (x != last_x || y != last_y) {
            if (last_x >= 0) draw_cursor(last_x, last_y, 0x01); // затираем старый цветом фона
            draw_cursor(x, y, 0x0F); // рисуем новый (белый)
            last_x = x;
            last_y = y;
        }

        // ESC (scancode 0x01) - выход в стоп-цикл. Опрашиваем порт
        // прямо здесь, без IRQ1 - клавиатурное прерывание этой демке
        // не нужно ни для чего другого.
    ...
    fill_screen(0x04); // красный фон - визуальный сигнал "вышли из цикла"
    while (1) {
