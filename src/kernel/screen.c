...
// физическим адресам, а не в .bss.
//
// НАЙДЕН И ИСПРАВЛЕН РЕАЛЬНЫЙ БАГ: раньше TTYS_BASE=0x158000 лежал ВНУТРИ
// буфера разбора ELF (ELF_STAGING_BASE=0x150000, kernel.c, размер
// ELF_STAGING_SIZE=0xFA00 -> занимает [0x150000, 0x15FA00)) - оба массива
// ttys[0]/ttys[1] (sizeof(tty_state)*2 ~ 8080Б) целиком помещались в этот
// диапазон. Каждый `run <file>` (в т.ч. автозапуск SH.BIN при каждой
// загрузке и при Ctrl+Alt+F1/F2) грузит файл через vfs_read() прямо в
// elf_staging_buf; любой файл БОЛЬШЕ 0x158000-0x150000=32768 байт (SH.BIN
// сам весит ~34КБ) дописывался поверх живых данных ttys[0].buffer.
// Повреждение не было видно немедленно (реальная VGA-память 0xB8000 не
// затрагивалась), но "стреляло" при следующем tty_switch() (screen.c,
// Ctrl+Alt+F1/F2) - подтверждено вживую: после `run SH.BIN` на второй
// консоли переключение назад на консоль 0 показывало мусор именно в
