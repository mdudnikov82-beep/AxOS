...
void kernel_main(unsigned long hart_id, unsigned long dtb) {
    ...
    unsigned long pmem_start = (unsigned long)_kernel_end;

    // KASLR: съедаем случайное число страниц СРАЗУ после конца ядра,
    // прежде чем отдать pmem_init() реальную нижнюю границу - куча
    // (первый и самый частый потребитель alloc_page(), см. heap.c)
    // начинается с ПЕРВОЙ страницы, которую вернёт pmem, так что этот
    // сдвиг и определяет её стартовый адрес на этой конкретной загрузке.
    //
    // Сознательно НЕ развязываем виртуальный адрес кучи от физического
    // (в отличие от первой попытки на x86-стороне, которая для этого
    // заводила отдельные PML4/PDPT/PD цепочки) - тот путь на x86-стороне
    // упёрся в реальный double fault при переключении задач (current_task
    // сам лежит в куче, а schedule() читает его прямо посреди смены CR3;
    // причину не нашли за разумное время, откатили). Тут риск такой же
    // (структуры процессов proc.h тоже живут в куче через kmalloc), а
    // выигрыш в энтропии всё равно был бы упёрт в те же физические 128МБ
    // RAM по умолчанию - так что просто сдвигаем СТАРТ бампового
    // аллокатора внутри уже единственной, всегда identity-mapped gigapage
    // (map_gigapage(0x80000000,...) в paging.c) - ноль новых таблиц
    // страниц, ноль риска той же природы.
    unsigned long t;
