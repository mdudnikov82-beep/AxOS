...
int elf_load(const char *filename) {
    ...
    /* User stack: one page just below 0x80000000. */
    void *stack_phys = alloc_page();
    if (!stack_phys) { uart_puts("[elf] OOM for stack\r\n"); return -1; }
    map_page_4k_pt(pt, USER_STACK_VA, (unsigned long)stack_phys,
                   PTE_V | PTE_R | PTE_W | PTE_U | PTE_A | PTE_D);

    unsigned long entry = hdr->e_entry;
    unsigned long usp   = USER_STACK_VA + PAGE_SIZE - 16;
    ...
    procs[pid].heap_brk = (heap_end + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
