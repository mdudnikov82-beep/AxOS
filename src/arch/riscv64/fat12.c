...
// Кешируем весь том в RAM (8192 сект. × 512 = 4 МБ).
// I/O через virtio_blk — меняем только два места по сравнению с x86-версией.
// Было 2048/1МБ - поднято до 8192/4МБ вместе с make_fat12_rv64.py (см.
// комментарий там: --emit-relocs раздул .elf-файлы, образ переполнился).
// bytes_per_sector/sectors_per_cluster/root_entries и т.д. читаются из
// самого BPB в рантайме - этот компайл-тайм размер только для RAM-кеша.

#define FAT12_TOTAL_SECTORS 8192
